var select_location = document.getElementById("selectDock");
    var options = ['River Street , Clerkenwell', 'Phillimore Gardens, Kensington', 'Christopher Street, Liverpool Street', "St. Chad's Street, King's Cross", 'Sedding Street, Sloane Square', 'Broadcasting House, Marylebone', "Charlbert Street, St. John's Wood", 'Maida Vale, Maida Vale', 'New Globe Walk, Bankside', 'Park Street, Bankside', 'Brunswick Square, Bloomsbury', 'Malet Street, Bloomsbury', 'Scala Street, Fitzrovia', "Belgrove Street , King's Cross", 'Great Russell Street, Bloomsbury', 'Cartwright Gardens , Bloomsbury', 'Hatton Wall, Holborn', 'Drury Lane, Covent Garden', 'Taviton Street, Bloomsbury', 'Drummond Street , Euston', 'Hampstead Road (Cartmel), Euston', 'Northington Street , Holborn', 'Red Lion Square, Holborn', 'British Museum, Bloomsbury', 'Doric Way , Somers Town', 'Ampton Street , Clerkenwell', 'Bouverie Street, Temple', 'Bolsover Street, Fitzrovia', 'Hereford Road, Bayswater', 'Windsor Terrace, Hoxton', 'Fanshaw Street, Hoxton', 'Leonard Circus , Shoreditch', 'Central House, Aldgate', "Pancras Road, King's Cross", 'De Vere Gardens, Kensington', "Penywern Road, Earl's Court", 'Abingdon Villas, Kensington', 'Shoreditch High Street, Shoreditch', 'Commercial Street, Shoreditch', 'Pindar Street, Liverpool Street', 'Wenlock Road , Hoxton', 'Crawford Street, Marylebone', 'Bruton Street, Mayfair', 'Boston Place, Marylebone', 'Nesham Street, Wapping', 'Warwick Avenue Station, Maida Vale'];
...
//CSV BAR DATA
function parseData_2(createBar){
    Papa.parse("https://raw.githubusercontent.com/sanish96/DE4-SIOT/master/data/data_av_timesplit.csv", {
        download: true,
        complete: function(results){
...
function createBar(data_hours){
    console.log(data_hours)

    var average_trends = document.getElementById('bar_trends')
    ...
    for (var i = 1; i <24; i++){
        bike.push(data_hours[28][3*i-2]);
        temp.push(data_hours[28][3*i-1]);
        wind.push(data_hours[28][3*i]);
    ...
    var chart_raw_temp = new Chart(average_trends,{
    type: 'bar',
    data: {
    ...
    options: {
      title: {
        display: false,
        text: 'Average Data Collected by Hour'
      }
    }
});
}
parseData_2(createBar)
